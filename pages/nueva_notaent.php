...
<head>
    ...
    <title>Nota Entrega (Clientes)</title>
    <!-- Bootstrap Core CSS -->
...
{ 
  ...
  if ($("#hddcontador").val()=="0")
  {
    ...
    dir_url = "registrar_notaent_db.php";
    $.ajax({
           type: "POST",
           url: dir_url,
           ...
           {          
		         ...
		         if (data>0)
			       {
                var chkdivisa = 'N';
                if ($('#chkdivisa').prop('checked')) {
                    chkdivisa = $('#chkdivisa').val();
                }

  			        alert("Registrado Correctamente!");
  			        location.href = "rep_nota_ent1.php?id_nota="+data+"&chkdivisa="+chkdivisa;
			       }
		         else
		         {
...
if (valida_repetido($("#cbofkproducto").val())){
	...
	if (valida_monedas_diferentes($("#hddmnd").val())){
        alert ("Los precios deben expresados en la misma moneda");
        return;
    }
...
if ($("#cbocant").val()=="BLT.")
 ...
 if ($("#cbocant").val()=="UNID.")
  ...
  var valor_col7=$("#hddmnd").val();
  var valor_col8=$("#hddexceptoiva").val();
  ...
  var ctrl_col7="<input name='mnds[]' type='hidden' value='" + valor_col7 + "'/>";
  var ctrl_col8="<input name='exceptoiva[]' id='exceptoiva_"+cont+"' type='hidden' value='" + valor_col8 + "'/>";
  ...
  tabla=tabla+"<td width='5%'>"+ctrl_col6+ctrl_col7+ctrl_col8+"<INPUT type='button' value='-' class='btn btn-primary' onclick='$(this).parent().parent().remove(); elimFilaProd("+valor_col5+");'/><td>";
  tabla=tabla+"<tr>";
...
function Calc_Total(excent){
  ...
  var subtotal = parseFloat($("#txtsubTotal").val());
  var ivatotal = parseFloat($("#txttotaliva").val());
  $("#txtTotal").val(subtotal+ivatotal-excent);
}
//(*--------------------------------------------*)
      ...
      $.ajax(url).done(function(data)
       {
          ...
          {     
			...
			//document.getElementById('txtsaldorest').value =$("#txtsaldorest").val();
...
//(*--------------------------------------------*)
function aplica_vendedor()
{
	if ($("#cbofkvendedor").find('option:selected').text()=='N/A' || $("#cbofkvendedor").val()=='' || $("#cbofkvendedor").val()=='null' || $("#cbofkvendedor").val()=='NULL')
	{
		$("#txtprecioVend").attr('readonly','true');
		$("#txtprecioVend").val('0.00');
	}
	else
	{
		$("#txtprecioVend").attr('readonly','');
		if ($("#txtprecio").val()!='')
			$("#txtprecioVend").val($("#txtprecio").val());
		else
			$("#txtprecioVend").val('0.00');
	}
}
...
<div class="portada">
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Nota de Entrega</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            Elementos Basicos para la Nota de Entrega para un Cliente Determinado (Venta)
                        </div>
...
<table class="table" width="90%">
  <tbody>
    ...
    <tr>
        ...
        <td width="80%"><select id="cbofkvendedor" name="cbofkvendedor" onchange='aplica_vendedor()' class="form-control"></select></td>
    </tr>
    <tr>
        ...
        <tbody>
            ...
            <tr>    
               ...
               <td><div class="form-group input-group">
                          ...
                          <span id="spanprecioVend" class="input-group-addon">&nbsp;</span>
                      </div></td>
                ...
                <table width="100%"  class="table table-striped">
                    <thead>
                        ...
                        <tr>                     
                            <td>&nbsp;</td>
                            <td>&nbsp;</td>
                            <td style="text-align: right;"><label class="control-label">TOTAL NETO:</label></td> 
                            <td><div class="form-group input-group">
